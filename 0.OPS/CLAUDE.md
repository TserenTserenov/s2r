...
## Часть 3.5: Режим Repository-First (Гипотезы до поиска)

> **КРИТИЧЕСКИ ВАЖНО:** Claude НЕ отвечает "из головы". Ответы формируются **только** на основе документов репозитория и профиля пользователя.

### Почему это важно?

При работе с S2R репозиторием Claude должен действовать как **ассистент репозитория**, а не как "общеизвестный эксперт". Это означает:
- **НЕ** изобретать термины и структуры, которых нет в документации
- **НЕ** давать финальные ответы до изучения релевантных документов
- **Всегда** формировать гипотезы → искать → собирать Context Pack → отвечать

### 1.0 Интерпретация запроса: гипотезы до поиска

Перед любым поиском Claude ОБЯЗАН:

#### 1.0.1 Сформулировать гипотезы понимания запроса

**Формат:**
```
Гипотеза 1: Пользователь имеет в виду X (уверенность: высокая/средняя/низкая)
Гипотеза 2: Пользователь имеет в виду Y (уверенность: ...)
```
...
#### 1.0.2 Определить план поиска документов

На основе гипотез определить:
- Какие документы точно релевантны
- Какие документы возможно релевантны
- Какие ядра и семейства затрагивает запрос

#### 1.0.3 Запросить уточнение при низкой уверенности

Если максимальная уверенность ≤ "средняя", задать **один** уточняющий вопрос пользователю перед поиском.

### Обязательный порядок работы

> **ЗАПРЕТ:** Нельзя давать финальный ответ до выполнения шагов 1.0-1.2.

```
1.0 Интерпретация → Гипотезы понимания запроса
1.1 Поиск → Найти релевантные документы
1.2 Context Pack → Собрать пакет контекста (5-15 выдержек)
1.3 Результат → Дать ответ на основе Context Pack
```

### Формат Context Pack

При ответе на сложные вопросы формируй Context Pack:

```
## Context Pack для [запрос]

### Использованные источники:
1. `0.1.Knowledge-Logic/02-document-families.md:119-180` — описание семейств FX4-FX6
2. `0.3.Roles-Matrix-3x3/roles-matrix.md:145-177` — роль FX4 (Визионер ЦС)
3. ...
...
- Документ X — не релевантен, т.к. описывает другое ядро
```
...
│   │   ├── 01-formal-checks.md    # Формальные проверки
│   │   ├── 02-content-checks.md   # Содержательные проверки
│   │   └── 03-development-analysis.md  # Анализ развития
...
> **ОБЯЗАТЕЛЬНО:** При переименовании файла/папки или изменении структуры синхронизируй связанные документы.

```
□ Обновил 0.6.Repository-Processes/03-structure.md?
□ Обновил CLAUDE.md (раздел "Структура 0.OPS/")?
...
□ Обновил 08-deployment-guide.md (таблицы файлов)?
□ Обновил README.md ядра (если затронуто)?
□ Проверил все ссылки на переименованные файлы?
```

> **Правило именования:** README файлы — без номеров (`README.md`). Содержательные файлы — с номером по значимости (`01-name.md`).
