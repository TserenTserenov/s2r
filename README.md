...
## Для чего SRT-метод (Job to be Done)

SRT нужен, когда в проекте появляется типичная боль:

- **Знания расползаются** по «папкам по настроению» и «заметкам без адреса»
- **Разные роли читают разные документы**, но нет карты «что где лежит»
- **Требования, архитектура, процессы и команда** описываются несвязно
- **Сложно подключать новых людей и ИИ-агентов** к репозиторию как к «единому источнику правды»

**SRT превращает хранилище в контролируемую модель проекта**, где каждый документ имеет «координаты» (какая система × для какой роли).

## Чем SRT НЕ является

- Это **не процесс разработки** и не «жизненный цикл проекта»
- Это **схема организации знаний** + минимальная операционная дисциплина ведения репозитория (метасемейство управления F0)
- Это **не замена архитектурным фреймворкам**, а способ разложить их результаты по адресам
